...
function mostrar(idProd){
	$.post("../ajax/inventario.php?op=mostrar", {idProd:idProd}, function(data){
		console.log(data);
		data=JSON.parse(data);
		mostrarform(true);
		$('#idProd').val(data.idProd);
...
function desactivar(idProd){
	...
	$('#rsptaSi').click(function () {
		...
		$.post("../ajax/inventario.php?op=desactivar", {idProd:idProd}, function(mensaje){
			valida= mensaje.indexOf('rror');
			if(valida!=-1){
				toastr["error"](mensaje);
			}else{
...
function activar(idProd){
	...
	$('#rsptaSi').click(function () {
		...
		toastr.clear(ventanaEleccion);
		$.post("../ajax/inventario.php?op=activar", {idProd:idProd}, function(mensaje){
			valida= mensaje.indexOf('rror');
			if(valida!=-1){
				toastr["error"](mensaje);
			}else{
